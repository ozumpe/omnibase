...
- any other value (``claude``, ``llm``, …): calls a real LLM via the
  ...
  source + baseline, and returns a fully type-annotated optimised version. The
  candidate then goes through the full gauntlet exactly like the stub's — the
  LLM is never trusted, the deterministic gates are.
...
def propose(current_source: str, baseline_latency: float) -> str:
    ...
    return _llm_proposal(current_source, baseline_latency)
...
def _llm_proposal(current_source: str, baseline_latency: float) -> str:
